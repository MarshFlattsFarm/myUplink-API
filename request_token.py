#
#   This is a Python script to retrieve an OAuth2 Access Token for later use with the myUplink API
#   The allocated token is written to a file called .myUplink_API_Token.json in the user's home directory
#
...
#
#   Pre-requisites:
#     - The myUplink API website must have been used to register an Application (i.e. the script that will call the API)
...
query_scope = 'READSYSTEM offline_access'
unique_state = 'x'
...
#     - The 'scope' is the type of permission we want to request (specfic to the API being called)
#     - The 'redirect_uri' is what we want to invoke as a call-back
#     - The 'state' is the secret string we specify when calling the Authorization URL; the docs suggest 'x'
myuplink = OAuth2Session(client_id=client_id, scope=query_scope, redirect_uri=redirect_url, state=unique_state)
...
if len(authorization_code) < 60:
    raise SystemExit('Invalid Authorization Code entered; exiting')

#   Request a Token from token_url, using the entered Code
token = myuplink.fetch_token(token_url=token_url, code=authorization_code, include_client_id=True, client_secret=client_secret)
